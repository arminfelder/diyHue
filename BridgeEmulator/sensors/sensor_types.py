...
# MODEL_ALIASES maps alternative model IDs to a canonical entry in sensorTypes.
# (RDM002 reports its model id directly, so only the dimmer-switch variants need
# aliasing.) Exported so the MQTT service can stay in sync without duplication.
MODEL_ALIASES = {
    "RWL020": "RWL021",   # Hue dimmer switch (older EU hardware revision)
    "RWL022": "RWL021",   # Hue dimmer switch (newer hardware revision)
}

# Sensor types that are sub-components of another sensor's CLIP v2 device. They
# share a uniqueid with their primary partner and are linked via parent_id_v2.
SUB_SENSOR_TYPES = {"ZLLRelativeRotary"}

for _alias, _canonical in MODEL_ALIASES.items():
    if _canonical in sensorTypes:
