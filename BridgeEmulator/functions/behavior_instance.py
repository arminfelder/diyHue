...
def checkBehaviorInstances(device):
    ...
    for instance in matchedInstances:
        if device.type == "ZLLSwitch" and "button1" in instance.configuration: # New per-button format (RDM002 / H56)
            ...
            # Mirror the real bridge: dim by the device's own per-detent step with
            # a short ramp; at the bottom hold minimum then turn off; dialling up
            # from off recalls the last on-state rather than jumping to minimum.
            direction = device.state.get("direction", "clock_wise")
            step = device.state.get("rotary_step_size", 8)
            transition = device.state.get("rotary_transition", 4)  # centiseconds (~device's action_transition_time)
            rotary_cfg = instance.configuration.get("rotary", {})
            for resource in rotary_cfg.get("where", []):
                if "group" not in resource:
                    continue
                group = findGroup(resource["group"]["rid"], resource["group"]["rtype"])
                if group is None:
                ...
                if direction == "counter_clock_wise":  # dim down
                    current_bri_pct = state.get("avr_bri", 0) if any_on else 0
                    if not any_on or current_bri_pct <= 1:
                        # already at minimum (or off) -> run on_dim_off
                        if rotary_cfg.get("on_dim_off", {}).get("action") == "all_off":
                            group.setV1Action({"on": False})
                    else:
                        group.setV1Action({"bri_inc": -step, "transitiontime": transition})
                else:  # dim up
                    if not any_on:
                        # on_dim_on: recall the last on-state (last brightness), the
                        # lights remember their level, so just switch them back on
                        group.setV1Action({"on": True, "transitiontime": transition})
                    else:
                        group.setV1Action({"bri_inc": +step, "transitiontime": transition})
